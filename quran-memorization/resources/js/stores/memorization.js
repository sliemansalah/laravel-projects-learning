...
export const useMemorizationStore = defineStore('memorization', () => {
    ...
    async function startAyah(ayahId) {
        ...
        try {
            ...
            error.value = err.response ? .data ? .message || 'حدث خطأ في بدء الحفظ'
            throw err
        } finally {
            loading.value = false
        }
    ...
    async function submitWord(wordId, isCorrect) {
        ...
        try {
            ...
            error.value = err.response ? .data ? .message || 'حدث خطأ في حفظ الكلمة'
            throw err
        } finally {
            loading.value = false
        }
    ...
    async function getTodayReview() {
        ...
        try {
            ...
            error.value = err.response ? .data ? .message || 'حدث خطأ في تحميل المراجعة'
            throw err
        } finally {
            loading.value = false
        }
    ...
    async function getProgress() {
        ...
        try {
            ...
            error.value = err.response ? .data ? .message || 'حدث خطأ في تحميل التقدم'
            throw err
        } finally {
            loading.value = false
        }
    ...
    async function completeSession() {
        ...
        try {
            ...
            error.value = err.response ? .data ? .message || 'حدث خطأ في إكمال الجلسة'
            throw err
        } finally {
            loading.value = false
        }
